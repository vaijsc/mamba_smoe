...
class FMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def __init__(
        ...
        self.weights = nn.Linear(self.d_model, 2 * self.d_model)
        # self.weights_out = nn.Linear(2 * self.d_model, self.d_model)
    ...
    def forward(self, moe_inp):
        ...
        torch.Size([2048, 128])
        """
        transformed_inp = torch.nn.functional.softplus(self.weights(moe_inp))
        gate_top_k_idx, gate_score = self.gate(transformed_inp)
        """
