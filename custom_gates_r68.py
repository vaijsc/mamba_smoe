    ...
    def forward(self, inp, return_all_scores=False):
        ...
        if self.dense_moe_flag:
            ...
            gate_top_k_val_2, gate_top_k_idx_2 = torch.topk(
                torch.transpose(gate_2_sinkhorn, 1, 0), k=expert_top_k, dim=-1, largest=True, sorted=False
            )
            gate_top_k_val_2 = torch.transpose(gate_2, 1, 0)[torch.arange(self.tot_expert, device=gate_2.device).unsqueeze(1), gate_top_k_idx_2]
