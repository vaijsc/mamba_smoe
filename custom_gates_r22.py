...
class CustomNaiveGate_Balance_SMoE(BaseGate):
    ...
    def forward(self, inp, return_all_scores=False):
        ...
        score_rate = F.softmax(gate, dim=-1)
        score_rate = score_rate.sum(dim=0)
        ...
        if self.dense_moe_flag:
            ...
            gate_top_k_val_2, gate_top_k_idx_2 = torch.topk(
                torch.transpose(gate_exp, 1, 0), k=expert_top_k, dim=-1, largest=True, sorted=False
            )  # [.. x top_k] 
        ...
        gate_idx_exp = gate_idx_exp.long().to(device)
        # import ipdb; ipdb.set_trace()
        ...
        ### modify
        return gate_top_k_idx_1, gate_score_1, gate_top_k_idx_2, gate_score_2, gate_idx_exp
