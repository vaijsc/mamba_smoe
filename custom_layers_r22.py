...
import tree
from custom_functions_r22 import prepare_forward, ensure_comm, prepare_forward_expert_choice
from custom_functions_r22 import MOEScatter, MOEGather
from custom_functions_r22 import AllGather, Slice
...
def _fmoe_general_global_forward(
    ...
    * Gather the output features of experts back, and reorder them as sentences.
    ...
    """
    (
        pos, # size [2 * num_tokens], each cluster show token come to experts
        local_expert_count, # w.r.t each expert, how many tokens go through
        global_expert_count, # 
        fwd_expert_count,
        fwd_batch_size,
...
def _fmoe_expert_choice_general_global_forward(
    inp, gate, expert_fn, num_expert, world_size, gate_idx_exp, **kwargs
):
    r"""
    A private function that performs the following steps to complete the MoE
    computation.
    ...
    * Gather the output features of experts back, and reorder them as sentences.
    ...
    (
        ...
        fwd_expert_count,
        fwd_batch_size,
    ) = prepare_forward_expert_choice(gate, num_expert, world_size, gate_idx_exp)
    # gate ~ gate_idx_top_k_2, num_expert = 16, world_size=1, 
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def expert_fn(self, inp, fwd_expert_count):
        ...
        if self.experts_fused:
            ...
            expert_mask[fwd_expert_count > 0] = 1.0
    ...
    def forward(self, moe_inp):
        ...
        # import ipdb; ipdb.set_trace()
        gate_top_k_idx_1, gate_score_1, gate_top_k_idx_2, gate_score_2, gate_idx_exp = self.gate(moe_inp)
        """
        ...
        fwd_1 = _fmoe_general_global_forward(
            moe_inp,
            gate_top_k_idx_1, # gate_top_k_idx_1.unique()
            self.expert_fn,
            self.num_expert,
            self.world_size,
