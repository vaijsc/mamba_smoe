...
class FMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def forward(self, moe_inp):
        ...
        gate_weight1 = (torch.sigmoid(self.weights(moe_inp)) > 0.5).float().to(device)
        ...
        gate_score_1_ = gate_score_1_.view(-1, 1, self.top_k)
        gate_score_2_ = gate_score_2_.view(-1, 1, self.top_k)
        ...
        # Create an empty result tensor and reassign based on masks
        moe_outp = moe_outp_1 + moe_outp_2
        if self.slice_size > 1:
