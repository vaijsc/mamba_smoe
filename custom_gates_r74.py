...
def sinkhorn_knopp(B, num_iterations=100, tol=1e-2, device="cuda", eps=2):
    """
    ...
    B = B.to(device)
    T, E = B.shape
    K = torch.exp(B /2 - 1)  # Gibbs kernel
...
## handle inp_2
        ...
        if self.dense_moe_flag:
            ...
            gate_top_k_val_1, gate_top_k_idx_1 = torch.topk(
                gate_1, k=self.top_k, dim=-1, largest=True, sorted=False # r70
            )  # [.. x top_k] 
            # import ipdb; ipdb.set_trace()
