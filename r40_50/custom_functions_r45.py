...
def prepare_forward_expert_choice(gate, num_expert, world_size):
    ...
    with torch.no_grad():
        ...
        # local_expert_count_second_part = torch.full((num_expert,), num_token_per_expert, dtype=int)
        local_expert_count = torch.cat([torch.full((num_expert,), num_token_per_expert, dtype=int)]).contiguous()
