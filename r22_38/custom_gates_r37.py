    ...
    def forward(self, inp, return_all_scores=False):
        ...
        if self.dense_moe_flag:
            ...
            gate_top_k_val_1, gate_top_k_idx_1 = torch.topk(
                gate[:, : 4], k=self.top_k, dim=-1, largest=True, sorted=False
            )  # [.. x top_k] 
            
            gate_top_k_val_2, gate_top_k_idx_2 = torch.topk(
                torch.transpose(gate[:, 4 :], 1, 0), k=expert_top_k, dim=-1, largest=True, sorted=False
            )
        ...
        if self.g_blance:
            self.loss = self.set_load_balance(gate[:, : 4], gate_top_k_idx_1)
