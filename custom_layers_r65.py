...
import tree
from custom_functions_r65 import prepare_forward, prepare_forward_expert_choice ,ensure_comm
from custom_functions_r65 import MOEScatter, MOEGather
from custom_functions_r65 import AllGather, Slice
...
class FMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def forward(self, moe_inp):
        ...
        # import ipdb; ipdb.set_trace()
        moe_inp_1, gate_top_k_idx_1, gate_score_1 = self.gate(moe_inp)
        """
        ...
        moe_outp = tree.map_structure(expert_combine_func, num_token * self.h, gate_top_k_idx_1, gate_score_1, moe_outp_1)
        # moe_outp = torch.cat([moe_outp_2, moe_outp_1], dim=-1)
        # moe_outp = self.weights(moe_outp)
        moe_outp = moe_outp.reshape(moe_outp.size(0) // self.h , self.h , moe_outp.size(1)).reshape(moe_outp.size(0) // self.h, -1)
